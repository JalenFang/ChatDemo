...
public class AreaChart extends LnChart {
    ...
    protected void drawClipPlot(Canvas canvas) {
        if (renderVerticalPlot(canvas) == true) {
            // 画横向定制线
            if (null != mCustomLine) {
                mCustomLine.setVerticalPlot(dataAxisRender, plotArea, getAxisScreenHeight());
        ...
        if (count == 3) {
            ...
            if (areaData.getApplayGradient()) {
                ...
                if (areaData.getGradientDirection() == XEnum.Direction.VERTICAL) {
                    ...
                    linearGradient = new LinearGradient(0, 0, 0,
    ...
    private boolean renderArea(Canvas canvas, Paint paintAreaFill, Path pathArea, AreaData areaData, List<PointF> lstPathPoints) {
        ...
        if (areaData.getApplayGradient()) {
            ...
            if (areaData.getGradientDirection() == XEnum.Direction.VERTICAL) {
                float lineMaxY = getLineMaxMinY(Y_MAX, lstPathPoints);
                linearGradient = new LinearGradient(0, 0, 0, lineMaxY, areaData.getAreaBeginColor(),//
                        areaData.getAreaEndColor(), areaData.getGradientMode());
            } else {
                float lineMinY = getLineMaxMinY(Y_MIN, lstPathPoints);
                linearGradient = new LinearGradient(plotArea.getLeft(), plotArea.getBottom(), lstPathPoints.get(count - 1).x, lineMinY,//
    ...
    private boolean renderDotAndLabel(Canvas canvas, AreaData bd, int dataID, List<DotInfo> lstDotInfo) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!pLine.getDotStyle().equals(XEnum.DotStyle.HIDE)) {
                ...
                savePointRecord(dataID, i, dotInfo.mX + mMoveX, dotInfo.mY + mMoveY, dotInfo.mX - radius + mMoveX,//
                        dotInfo.mY - radius + mMoveY, dotInfo.mX + radius + mMoveX,//
