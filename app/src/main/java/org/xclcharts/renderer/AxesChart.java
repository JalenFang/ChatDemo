...
public class AxesChart extends EventChart {
    ...
    public AxesChart() {
        Log.i("dragon", "AxesChart Create");
        if (null == mLstDataTick) {
            mLstDataTick = new ArrayList<>();
        }

        if (null == mLstCateTick) {
            mLstCateTick = new ArrayList<>();
        }
    ...
    public void initDataAxis() {
        if (null == dataAxisRender) {
            dataAxisRender = new DataAxisRender();
        }
    }
    ...
    protected void drawDataAxisLabels(Canvas canvas,
        ...
        for (int i = 0; i < lstLabels.size(); i++) {
            ...
            switch (mDataAxisLocation) {
                ...
                case RIGHT:
                case VERTICAL_CENTER:
                    dataAxisRender.renderAxisHorizontalTick(this.getLeft(), this.getPlotArea().getLeft(),
                ...
                case BOTTOM:
                case HORIZONTAL_CENTER:
                    dataAxisRender.renderAxisVerticalTick(canvas, t.X, t.Y, t.Label,
    ...
    protected void drawHorizontalGridLines(Canvas canvas, float plotLeft, float plotRight,
        ...
        if (tickID >= 0 && tickID < tickCount) {
            plotGrid.setPrimaryTickLine(dataAxisRender.isPrimaryTick(tickID));
