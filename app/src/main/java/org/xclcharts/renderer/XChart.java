...
public class XChart implements IRender {
    ...
    protected void renderBorder(Canvas canvas) {
        if (mShowBorder) {
            ...
            mBorder.renderBorder("BORDER", canvas, mLeft, mTop, mRight, mBottom);
        }
    }
    ...
    protected void renderChartBackground(Canvas canvas) {
        if (this.mBackgroundColorVisible) {
            ...
            if (mShowBorder) {
                ...
                mBorder.renderBorder("CHART", canvas, mLeft - borderSpadding, mTop - borderSpadding,//
    ...
    protected boolean getClikedScaleStatus() {
        ...
        //如果在这范围内，则可以处理点击
        if (Float.compare(mXScale, 0.95f) == 1 && Float.compare(mXScale, 1.1f) == -1) {
            return true;
        } else {
            return false;
        }
    ...
    private void scaleChart(Canvas canvas) {
        ...
        if (Float.compare(mCenterX, 0.0f) == 1 || Float.compare(mCenterY, 0.0f) == 1) {
