...
public class LnChart extends AxesChart {
    ...
    public float getVPValPosition(double bv) {
        float vaxlen = (float) MathHelper.getInstance().sub(bv, dataAxisRender.getAxisMin());
    ...
    protected float getAxisYPos(XEnum.AxisLocation location) {
        if (dataAxisRender.getAxisStdStatus() && categoryAxis.getAxisBuildStdStatus()) {
            ...
            switch (pos) {
                ...
                case VERTICAL_CENTER:
                    ...
                    currentTickLabel = MathHelper.getInstance().add(
                            dataAxisRender.getAxisMin(), mul(i, (float) dataAxisRender.getAxisSteps()));
